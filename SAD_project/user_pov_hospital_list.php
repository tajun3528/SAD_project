...
if ($search !== '') {
  $stmt = $conn->prepare("SELECT * FROM hospitals WHERE name LIKE ? OR location LIKE ? OR recommendation LIKE ? ORDER BY created_at DESC");
  $searchTerm = '%' . $search . '%';
...
<body>
  <header>
    ...
    <div class="search-bar">
      <form method="GET" action="user_pov_hospital_list.php" style="display: flex; width: 100%;">
      ...
      <?php while($row = $result->fetch_assoc()): ?>
        <div class="hospital-card">
          ...
          <div class="hospital-info">
            <h3><?php echo highlight($row['name'], $search); ?></h3>
            <p><strong>Location:</strong> <?php echo highlight($row['location'], $search); ?></p>
            <p><strong>Recommendation:</strong> <?php echo highlight($row['recommendation'], $search); ?></p>
            <div style="margin-top: 10px;">
              <a href="user_pov_view_h.php?id=<?php echo $row['id']; ?>" class="btn btn-view">See Details</a>
              <a href="book_hospital.php?id=<?php echo $row['id']; ?>" class="btn btn-book">Book</a>
            </div>
          </div>
        </div>
      <?php endwhile; ?>
    <?php else: ?>
      <p style="text-align: center;">No hospitals found<?php echo $search ? ' for "' . htmlspecialchars($search) . '"' : ''; ?>.</p>
    <?php endif; ?>
  </div>
</body>
</html>

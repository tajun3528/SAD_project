...
if ($search !== '') {
  $stmt = $conn->prepare("SELECT * FROM hospitals WHERE name LIKE ? OR location LIKE ? ORDER BY created_at DESC");
  $searchTerm = '%' . $search . '%';
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
    ...
    .hospital-card { display: flex; gap: 20px; margin-bottom: 20px; background: #f9fafb; padding: 15px; border-radius: 10px; }
    .hospital-card img { width: 120px; height: 80px; object-fit: cover; border-radius: 5px; }
    .hospital-info h3 { margin: 0; color: #0f172a; }
    .hospital-info p { margin: 5px 0; color: #64748b; }
  ...
  <div class="main">
    ...
    <div class="form-section">
      ...
      <form action="add_hospital.php" method="POST" enctype="multipart/form-data">
        ...
        <textarea name="recommendation" rows="4" placeholder="Recommendation for Disease" required></textarea>
        <textarea name="details" rows="4" placeholder="Additional Details"></textarea>
        <input type="file" name="hospital_image" accept="image/*" required>
    ...
    <div class="form-section">
      ...
      <form method="GET" action="manage_hospitals.php">
      ...
      <?php if ($result->num_rows > 0): ?>
        <?php while($row = $result->fetch_assoc()): ?>
          <div class="hospital-card">
            <img src="<?php echo $row['image_path']; ?>" alt="Hospital Image">
            ...
            <div style="margin-top: 10px;">
              <a href="view_hospital.php?id=<?php echo $row['id']; ?>" class="btn btn-view">See Details</a>
              <a href="edit_hospital.php?id=<?php echo $row['id']; ?>" class="btn btn-edit">Edit</a>
              <a href="delete_hospital.php?id=<?php echo $row['id']; ?>" onclick="return confirm('Are you sure you want to delete this hospital?');" class="btn btn-delete">Delete</a>
            </div>
          </div>
        <?php endwhile; ?>
      <?php else: ?>
        <p>No hospitals found<?php echo $search ? ' for "' . htmlspecialchars($search) . '"' : ''; ?>.</p>
      <?php endif; ?>
    </div>
  </div>
</body>

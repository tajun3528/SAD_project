...
if ($search !== '') {
  $stmt = $conn->prepare("SELECT * FROM fruits WHERE name LIKE ? OR category LIKE ? OR recommendation LIKE ? ORDER BY created_at DESC");
  $searchTerm = '%' . $search . '%';
...
<body>
  <header>
    ...
    <div class="search-bar">
      <form method="GET" action="User_pov_fruits_list.php" style="display: flex; width: 100%;">
        <input type="text" name="search" placeholder="Search by fruit, category or disease name..." value="<?php echo htmlspecialchars($search); ?>" />
  ...
  <div class="main-content">
    ...
    <?php if ($result->num_rows > 0): ?>
        ...
        <div class="fruit-card">
          ...
          <div class="fruit-info">
            ...
            <div style="margin-top: 10px;">
              <a href="user_pov_view_f.php?id=<?php echo $row['id']; ?>" class="btn btn-view">See Details</a>
              <?php if ($row['quantity'] > 0): ?>
                <a href="add_to_cart.php?type=fruit&id=<?php echo $row['id']; ?>" class="btn btn-order">🛒 Add to Cart</a>
              <?php else: ?>
                <button class="btn btn-order" disabled>Out of Stock</button>
              <?php endif; ?>
            </div>
      ...
      <?php endwhile; ?>
    <?php else: ?>
      <p style="text-align: center;">No fruits found<?php echo $search ? ' for "' . htmlspecialchars($search) . '"' : ''; ?>.</p>
    <?php endif; ?>
  </div>
</body>
</html>